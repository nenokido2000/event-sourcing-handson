...
>
> **分析・設計の成果物を読むなら [`docs/README.md`](docs/README.md)（歩き方）から。** 目的別に3本の道を示しています。
...
- 🔄 M2: 倉庫の戦術設計（[`docs/tactical-design.md`](docs/tactical-design.md)）← 進行中。集約ごとのスライスで型レベルまで落とす
  - ✅ ① 在庫（`InventoryItem`／コア）… 引当明細・受付ゲート・例外・テスト骨子を確定。H8/H12 決着
  - ⬜ ② 入荷 / ③ 出荷 / ④ 棚卸 / ⑤ ポリシー・リードモデル
- ⬜ M3〜: 実装（TDD＋ATDD の二重ループ）以降、上記ロードマップに沿って進行

## セットアップ / 動作確認（M0 以降）

環境構築の前提ツール・手順・トラブルシュートは [`docs/setup.md`](docs/setup.md) を参照（新規に clone した人はまずこれを読む）。要点だけ:

- 必要なのは **JDK 25 / Docker Desktop / Git** の3つだけ（Gradle 本体は不要＝`./gradlew` が自動取得）。
- 固定版: Java 25 / Gradle 9.6.1 / Spring Boot 4.1.0 / Axon 4.13.2 / PostgreSQL 17 / DynamoDB Local。

```bash
docker compose -f infra/docker-compose.yml up -d   # DynamoDB Local + PostgreSQL
./gradlew build                                    # ビルド（初回は依存DL）
./gradlew test                                     # 全テスト
./gradlew :warehouse-app:bootRun                   # アプリ起動（ポート 8080 想定 / M3以降）
```
