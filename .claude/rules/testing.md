...
- **開発は TDD＋ATDD の二重ループで進める**（M3以降の実装フェーズ全体）。
  ...
  - 外側=ATDD: 受入基準を **Gauge の Markdown Spec（`specs/`）** に先に書き、**JDK 標準の `java.net.http.HttpClient`** でヘッドレスに REST を叩いて検証する（ブラウザ/UIは用意しない）。Spec は生きたドキュメントとして保つ。
...
- プロジェクションは「イベント入力 → リードモデル状態」のテストを書く。冪等性（同一イベントの二重適用）も検証する。
