...
> [`../ubiquitous-language.md`](../ubiquitous-language.md) が正**（例: P6 引当解放は M2 の出荷スライスで、
> P7 注文取消解放は M3-a 着手前の見直しで追加されたので、下の表と図にはいない
> → [H17](../decisions.md#h17-宙に浮いた引当を誰が解放するか) / [H46](../decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）。
...
> ※ 棚卸スライスで **`実地数量がカウントされた → 在庫差異が記録された`（イベント→イベント直結）という唯一の文法違反を解消**した（間にコマンドが不在だった）。
...
> **M2 以降の補足**: 「引当を解除する」の発生源はその後2度変わった。M2 で **P6 引当解放**（出荷が欠品完了／取消）が
> 加わり（[H17](../decisions.md#h17-宙に浮いた引当を誰が解放するか)）、M3-a 着手前に **P7 注文取消解放**が加わって
> **外部トリガから直接コマンドを出す経路は無くなった**（[H46](../decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）。
> 上表の「タイマー」も倉庫側には置かないと決着している（[H45](../decisions.md#h45-期限切れを誰が判定するか)）。
