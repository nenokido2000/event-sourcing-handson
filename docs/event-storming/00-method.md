...
3. ここで詰まる（❓ホットスポット H6）: **格納の*前*、受入ドックの在庫はロケーションが未確定**。
   在庫アイテムは「SKU×ロケーション」で身元が決まる集約なので、**ロケーション未確定の在庫を在庫アイテムに入れられない**。
4. →「ロケーションを持たない受入の器」が別に要る、と分かる。これが集約 **入荷ロット（`InboundReceipt`）** の発見。
5. 格納は「入荷ロットを減らす」と「在庫アイテムを増やす」の2つに割れる。**1トランザクション1集約**なので、
   これは1コマンドで両方を変えられない → イベント「在庫が格納された」に**反応するポリシー P1** が
   在庫アイテムへ「在庫を計上する」コマンドを送る（結果整合）。
...
2. ②の文法:
   - 🟦コマンド = 「引き当てる」。不変条件 = **引当可能（＝手持在庫−引当済）≥ 要求量**。これを守る責任者 = 在庫アイテム集約。
