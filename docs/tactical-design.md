...
- **P2・P5・P7 はリードモデルを読む**（引当先の選定・対象の列挙・解除対象の特定）。その配線は[リードモデル](#リードモデル)の図を参照。
...
|---|---|---|---|
| **棚卸の重複開始**（[H23](decisions.md#h23-棚卸の重複開始)） | 引当可能在庫ビューの `frozen`（`locationId IN (?) AND frozen`） | `DuplicateStocktakeException` | P5 の凍結が `AlreadyFrozenException` で弾き、**棚卸干渉ビュー**に落ちる |
...
- **★残る穴**: すり抜けた不正な明細でピッキングまで進むと、P3 の `IssueStock` が「未知の引当ID」として
...
> **P6 は出荷イベント由来なので、出荷指示より前に注文が取り消されると引当を解除できない**——
...
- 出荷指示済みの引当は **`CancelShipment` → P6** が解除する。取消の窓口は1つ（外部イベント）だが、
  **引当の状態によって行き先が分かれる**。
...
**リードモデルを読むポリシーは受注 BC 由来の2本**（あとは P5 が対象在庫の列挙で読む）。

### なぜ出荷指示済みを除くのか

無条件に解除すると、**ピッキング済みの引当が消える**。その後の `StockPicked` を受けた P3 が
...
- **★既知の穴**: 引当ビューの投影が遅れていると、P7 が引当を見つけられず**取りこぼす**。
  P2 は在庫集約が弾いて自己修正するが（H26）、P7 には弾く相手がいない——「引当が1件も無い」は
...
読み手: 出荷明細の組み立て（`orderLineId` で引く）／ピッキング担当の作業リスト（`shipmentId` で引く・[H41](decisions.md#h41-リードモデルの顔ぶれを絞った基準)）／P2 の再計画（H26）／**P7 の解除対象の特定**（`orderLineId` で引き `shipmentId` が null の行・[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）。
...
| 成果物 | 状態 |
|---|---|
| スライス①〜⑤ | 集約4つ・ポリシー6本・サーガ1本・リードモデル5種（冒頭の表）。**M2 完了時点の記録**——M3-a 着手前の見直しで P7 が加わった（[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)） |
