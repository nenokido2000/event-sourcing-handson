...
> | ⑤ ポリシー P1〜P6 / リードモデル | **確定**（2026-08-12。M3-a 着手前に **P7 を追加** → [H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)） |
...
（引当解放（P6）の追加・出荷取消・入荷クローズ・リードモデルの顔ぶれ）が載っていないため、ここに現在形を置く。
...
graph LR
    ...
    ORDX["注文が取り消された<br/>OrderCancelled（外部）"]:::ext
    ...
    ORDX -->|"P7 注文取消解放<br/>（出荷指示の無い引当を解除）"| INV
    ORDX -->|"出荷を取り消す（指示済みなら）"| SHP
...
- **在庫（コア）が受け側に集中している**。入口（格納伝播（P1）計上）・出口（出庫反映（P3）払出）・訂正（棚卸反映（P4）調整）・
  予約と解放（引当 P2・引当解放 P6・注文取消解放 P7）がすべて在庫へ集まる形が、コアサブドメインの位置をそのまま表している。
- **点線は Saga への確認の戻り**（棚卸凍結（P5）だけが往復する）。ほかのポリシーは行きっぱなしで状態を持たない。
- **引当（P2）・棚卸凍結（P5）・注文取消解放（P7）はリードモデルを読む**（引当先の選定・対象の列挙・解除対象の特定）。その配線は[リードモデル](#リードモデル)の図を参照。
- **受注 BC 由来が2本で対になっている**（P2 引当 / P7 注文取消解放）。どちらも外部イベントが倉庫の内部識別子を
  知らないため、引当ビューを読んで引き当て直す（[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）。
  取消は**出荷指示済みなら出荷側**（`CancelShipment` → 引当解放 P6）、**指示前なら在庫側**（注文取消解放 P7）へ分かれる。
...
flowchart TB
    ...
    X_CANCEL["↩ 注文が取り消された / 期限切れ<br/>(OrderCancelled)"]:::ext
    ...
    X_CANCEL -. 出荷指示済み .-> C_CANCEL --> E_CANCEL
...
- **見やすさのため3つ省いている**。①**投影の線**（どのイベントがどのビューを更新するか）は
  [リードモデルの配線図](#配線何が書き誰が読むか)が正。②**在庫元帳ビュー**は工程に関与しない（人が履歴を読むだけ）。
  ③**アクターが見るビュー**（次項）。
- **🟧イベントから👤アクターへ伸びる実線は「次の作業に取りかかる」の略記**。記法
  （[`event-storming/00-method.md`](event-storming/00-method.md)）ではアクターを動かすのは📄リードモデルの参照であって
  イベントではない。該当は4本（`在庫が入荷された→格納担当` / `出荷が指示された→ピッキング担当` /
  `在庫がピッキングされた→出荷担当` / `棚卸が開始された→棚卸担当`）。このうち**ピッキング担当が実際に見るのは
  引当ビュー**を出荷IDで引いたもの（[配線図](#配線何が書き誰が読むか)）。残る3人はビューを介さない——
  格納担当・棚卸担当は同じ伝票（`ReceiptId` / `StocktakeId`）の中で作業が続き、出荷担当は
  `ShipStock` に要る情報が出荷集約の中で確定しているため（[H41](decisions.md#h41-リードモデルの顔ぶれを絞った基準)）。
- **📄 から出る点線＝判断の入力**。ポリシーがリードモデルを読むのは引当（P2）（引当先の選定・再計画）・
  棚卸凍結（P5）（対象在庫の列挙）・注文取消解放（P7）（解除対象の特定）の3本。`StartStocktake` の前段検証（[H23](decisions.md#h23-棚卸の重複開始)）は
...
| 引き当てる（`AllocateStock`） | ○ 通す | 物理は動かない（H11。露出は引当（P2）が引当先を後回しにして下げる） |
...
#### 3. 引当を解除する（`DeallocateStock`）— 起点: ポリシー P6（引当解放）／ P7（注文取消解放）
...
- **発行元は2つ**（[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）。出荷指示済みなら P6（出荷イベント由来）、
  指示前なら P7（注文取消由来）。**経路が2本あっても壊れないのは H17 で冪等にしてあるから**。
...
#### 6-7. 凍結する／解凍する（`FreezeStock` / `UnfreezeStock`）— 起点: 棚卸凍結サーガ（P5）
...
- イベントに `sku` を載せるのは、**格納伝播ポリシー（P1）が入荷集約を引かずに `PlaceStock` を組み立てられる**ようにするため
...
- **`putAwayTotal`（格納累計）も載せる**。`(receiptId, putAwayTotal)` が**この格納を一意に指す**ので、
  ...
  出荷が `StockPicked` に `pickedTotal` を載せて出庫反映（P3）に渡すのと同型で、集約が既に持っている値
...
> 扱いは[単発伝播の3本（P1・P3・P4）](#ポリシー-p1p3p4単発伝播) の節を参照。
...
- **明細はコマンドに載せて渡す**。集約が引当ビュー（`AllocationView`）を引くことはしない
  （[`.claude/rules/cqrs-projection.md`](../.claude/rules/cqrs-projection.md)。引当先の選定は引当（P2）の責務で、
...
- **在庫IDではなく `(Sku, LocationId)` を持つ**（[H43](decisions.md#h43-bc-をまたぐ識別子の帰属)）。
  ...
  `InventoryItemId` を組み立てるのは出庫反映（P3）・引当解放（P6）の仕事（格納伝播（P1）が `sku` から組み立てるのと同型）。
- **明細の実在は[前段バリデーション](#前段バリデーション集約の外側)が確認する**（[H47](decisions.md#h47-前段バリデーションの枠組み)）。
  集約は引当ビューを引かないので、実在しない引当IDを含む指示を集約では弾けない。
  すり抜けると出庫反映（P3）の払出が黙って無視される（★残る穴。同節を参照）。
...
- **明細1件（引当1件）単位**。まとめて送らないので、出庫反映（P3）は `StockPicked` 1件 → `IssueStock` 1件の
  単発伝播のままでいられる（格納伝播（P1）と対称）。
- イベントに `sku` / `locationId` を載せるのは、**出庫反映（P3）が出荷集約を引かずに `IssueStock` を組み立てられる**ようにするため
...
- **明細を載せる理由**: 下流（リードモデル・**引当解放（P6）**）が出荷集約を引かずに処理できるようにするため。
...
- `cancelledLines` は**全明細**（まだ1件も消化していないため）。引当解放（P6）がこれを見て引当を解放する。
...
### 在庫集約との接続（出庫反映ポリシー P3 / 引当解放ポリシー P6）
...
**状態を持たないので Saga ではない**（棚卸凍結（P5）との違いは fan-out の有無ではなく途中状態の有無）。
...
- **予防はコマンドを受け付ける前段のバリデーション**（[H23](decisions.md#h23-棚卸の重複開始)）。
  ...
  → [前段バリデーション](#前段バリデーション集約の外側)の節。
  結果整合の参照なので **best effort**——すり抜けたぶんは棚卸凍結（P5）の凍結が弾き、棚卸干渉ビューに落ちる。
...
- **在庫IDを持たず `(Sku, LocationId)` で受ける**（[H43](decisions.md#h43-bc-をまたぐ識別子の帰属)）。
  ...
  数える対象は「棚とSKU」であって在庫集約の識別子ではない。組み立てるのは棚卸反映（P4）。
...
- **再開はできない**。中断した棚を改めて数えたければ**新しい棚卸を立てる**（棚卸凍結（P5）が解凍済みの棚を
  再凍結する制御を持たずに済む）。
- 状態遷移: `closed = true`／`closureReason = reason`
...
### 在庫集約との接続（棚卸反映ポリシー P4 / 棚卸凍結サーガ P5）
...
- **棚卸反映（P4）はカウントごとの即時伝播**（1イベント → 1コマンド。格納伝播 P1・出庫反映 P3 と同型）。**クローズ時の一括ではない**。
...
> ただし格納伝播（P1）・出庫反映（P3）とは原因が違い（棚卸 vs 棚卸の衝突）、**予防は `StartStocktake` の前段で重複開始を弾く**
...
> **集約の受付ゲートではない**（[H47](decisions.md#h47-前段バリデーションの枠組み)）。集約をまたぐ制約は集約では守れないので、
> コマンドを受け付ける前に `warehouse-app` がリードモデルを見て弾く。**性質は best effort**——
> 結果整合の参照なのですり抜けうる。上の各集約の例外一覧（強整合の保証）とは**別の表**にしてある。

| 弾くもの | 見るビュー | 例外 | すり抜けたら |
|---|---|---|---|
| **棚卸の重複開始**（[H23](decisions.md#h23-棚卸の重複開始)） | 引当可能在庫ビューの `frozen`（`locationId IN (?) AND frozen`） | `DuplicateStocktakeException` | 棚卸凍結（P5）の凍結が `AlreadyFrozenException` で弾き、**棚卸干渉ビュー**に落ちる |
| **出荷指示の明細**（[H47](decisions.md#h47-前段バリデーションの枠組み)） | 引当ビュー（`allocationId` で引く） | `InvalidShipmentLineException` | **無音の片落ちが残る**（下記） |

出荷指示の明細について確認するのは4点——**引当が存在する／状態が引当中／他の出荷に未割当（`shipmentId` が null）／
数量が引当量と一致する**。[H9](decisions.md#h9-出荷指示の出所と粒度) の「**引当が済んだものが**上流から指示される」を検査に落としたもの。

- **なぜ集約にやらせないか**: 出荷集約が引当ビューを引くことになり、
  [`cqrs-projection.md`](../.claude/rules/cqrs-projection.md) と H9（明細はコマンドに載って来る）に反する。
- **★残る穴**: すり抜けた不正な明細でピッキングまで進むと、出庫反映（P3）の `IssueStock` が「未知の引当ID」として
  **黙って無視される**（[H30](decisions.md#h30-ポリシーの二重発火にどう備えるか)）——在庫が減らないまま出荷が完了し、
  元帳にも干渉ビューにも痕跡が残らない。完全に塞ぐには在庫集約が未知の引当IDを拒否に変えるしかないが、
  それは H17・H30 が却下済み（「既に払い出し済み」と「そもそも存在しない」を集約は区別できない）。
- **逆方向のリスク**: 引当ビューの投影が遅れていると**正当な出荷指示を拒否しうる**（棚卸は「弾き損ねる」方向、
  出荷は「弾きすぎる」方向）。実運用では明細を引当ビューから組み立てる（[H41](decisions.md#h41-リードモデルの顔ぶれを絞った基準)）ので、
  ビューに載ってから指示が来る＝実質起きない。拒否されたら呼び出し側が再試行する。
- HTTP では `ProblemDetail` + `code`（例外の単純名）。どの明細が問題かは `detail` に書く（[H38](decisions.md#h38-受入ステップが叩く-rest-api-の契約)）。
...
> コア（P2 引当）は別節、1:N の P6（引当解放）も別節、状態を持つ棚卸凍結（P5）は Saga として別節。
...
- **どれも他集約・リードモデルを引かない**。組み立てに要る値はイベントに載っている
  （P1 の `sku` と `putAwayTotal`／P3 の `sku` `locationId` と `pickedTotal`）。ポリシーが読みに行くのは引当（P2）・棚卸凍結（P5）・注文取消解放（P7）の3本だけ。
- **3本とも在庫IDを組み立てる**（[H43](decisions.md#h43-bc-をまたぐ識別子の帰属)）。複合識別子を持つのは在庫 BC だけで、
  ...
  [H17](decisions.md#h17-宙に浮いた引当を誰が解放するか) の形（引当解放（P6）の語彙の写像）が、識別子にも及んでいる。
...
- **状態を持たない**。`@EventHandler` を持つだけの Spring Bean で、イベントごとに独立している
  （順序不問・どれか1件が失敗しても他に影響しない）。**この点だけが棚卸凍結サーガ（P5）との違い**。
- **プロジェクションと processing group を分ける**（[H30](decisions.md#h30-ポリシーの二重発火にどう備えるか)）。
  ...
  **ポリシーを巻き戻すと過去のコマンドが全部再発行される**。同じグループに同居させると、
  ...
  - このグループの意味は「**再構築してはいけない処理**」であり、ポリシー7本に加えて
    **棚卸干渉ビューの更新も属する**（[H48](decisions.md#h48-棚卸干渉ビューの更新をどのグループが担うか)）。
    再構築（truncate ＋ トークン巻き戻し）が触るのはプロジェクション側の4ビューだけ。
...
| 失敗の種類 | 例 | 扱い |
|---|---|---|
| **ドメイン上ありうる拒否** | `InventoryFrozenException`（格納伝播 P1・出庫反映 P3）／`NotFrozenByThisStocktakeException`（棚卸反映 P4） | **棚卸干渉ビューに1行書いて、イベントは消化する**。リトライしない（[H22](decisions.md#h22-凍結中に拒否された在庫反映の行き先)） |
...
- 干渉ビューへの書き込みは**ポリシーが直接行う**（拒否された事実はイベントストアに無い。H22・H28）。
  ここだけポリシーがリードモデルへ書くが、**イベントは発行しない**ので CQS は保たれている。
...
- **出庫反映（P3）・棚卸反映（P4）は受け側が冪等**なので、再送は差分ゼロになって消える。何もしなくてよい。
...
- 出荷指示（`RequestShipment`）は引当（P2）の仕事ではない。**引当が済んだものが上流から指示される**（H9）。
...
**途中状態がリードモデルから復元できるなら Saga は要らない**——棚卸凍結（P5）との分かれ目はここ。
...
- **凍結が引当を止めない**のは、引当が物理を動かさないから（H11）。
  格納伝播・出庫反映・棚卸反映（P1・P3・P4）が干渉ビューを必要としたのは「現場は動いたのに帳簿が取り残される」ためで、引当（P2）にはその形がない。
- **「絶対に取り損ねない」は買えない**（H26）。在庫が実際に足りなければ計画が減るだけで
  ＝**部分引当**に落ちる（例外にはならない・正常終了）。それが許されるのは、下流が欠品を扱えるから
  （[H15](decisions.md#h15-ピッキングの完了条件) / [H16](decisions.md#h16-欠品したまま終わる出荷の終わらせ方)）。
- **再処理に上限は設けない**（[H49](decisions.md#h49-p2-の再処理に上限を設けるか)）。例外はそのまま伝播させ、プロセッサの再配信に任せる。
  収束しないのは**プロジェクションが停止している**ときだけで、リトライが続くこと自体が異常の検知シグナルになる
  （H27 の終わらない Saga・H22 のポイズンピルと同じ姿勢）。上限で打ち切ると、
  **本来引けたはずの引当がシステムの不調を理由に失われる**。
- **再処理の単位は `OrderAccepted` イベント全体**（明細単位ではない）。成功済みの明細も再実行されるが、
  手順2で不足ゼロになり何も起きない。
- **同一 SKU の明細どうしは、厳密には独立ではない**（[H49](decisions.md#h49-p2-の再処理に上限を設けるか)）。
  引当ビューが結果整合なので、同じ `OrderAccepted` に同じ SKU の明細が2件あると2件目は1件目の引当が見えず、
  同じ棚を選ぶ。それでも壊れないのは**在庫集約が弾き（強整合）、再処理が復元する**ため。
- ただし [H24](decisions.md#h24-引当は受注の前か後か) により**注文は既に受け付け済み**なので、
  取り損ねは「果たせるはずの約束を果たせない」ことを意味する。**再計画を挟むのはこの重みのため**。

### 二重発火（H30）

引当（P2）は**三重に守られている**ので、`OrderAccepted` を2回処理しても引当は二重にならない。
...
**格納伝播（P1）と違って冪等の足場が集約側にある**（引当明細）ことが、ここでも効いている。
...
10. **同じ SKU の明細が2件ある注文**では、2件目が引当可能を超えて弾かれても、再処理で**不足ぶんだけ**引き直す
   （[H49](decisions.md#h49-p2-の再処理に上限を設けるか)。明細どうしが独立でなくなる唯一のケース）

**二重発火（H30）**
11. 同じ `OrderAccepted` を2回処理しても、2回目は同じ `AllocationId` になり在庫が二重に減らない
...
- 出荷イベントが明細（出荷実績明細 / `cancelledLines`）を持つ（③出荷スライス）ので、
  **引当解放（P6）は出荷集約を引かずに済む**。判定は**行ごとの `unshippedQty`**
  （[H44](decisions.md#h44-出荷イベントが持つ明細の形)）。

### 1:N だが状態を持たない（H17）

残明細の数だけコマンドを送るが、**各行は独立**（順序不問・1件失敗しても他に影響しない・途中経過を覚えない）。
**棚卸凍結（P5）との違いは fan-out の有無ではなく途中状態の有無**——この対比が「Saga と Policy を分けるのは状態の有無」を
はっきりさせた実例になっている。

### 語彙の写像

**BC をまたぐ語彙の翻訳は引当解放（P6）の責務**（出荷の言葉 → 在庫の言葉）。
...
- **`DeallocateStock` は冪等**（未知の引当IDならイベントを発行せず、例外も投げない。H17）。
  そもそも引当解放（P6）という自動発行元ができたことが、①在庫スライスで冪等にした理由そのもの。
- 一部を払い出した引当を解除すると、**戻るのは未払出残だけ**（イベントがその数量を載せる）。
- 凍結中でも通る（物理を動かさないため。H11）ので、**P6 も棚卸干渉ビューに書かない**（引当（P2）と同じ）。
...
## ポリシー P7 注文取消解放（`OrderCancellationPolicy`）

> ⑤ ポリシースライスの後に追加（[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）。
> **引当解放（P6）は出荷イベント由来なので、出荷指示より前に注文が取り消されると引当を解除できない**——
> [H17](decisions.md#h17-宙に浮いた引当を誰が解放するか) の「引当は必ず閉じる」に空いていた穴を塞ぐ。

### 入力と出力

```java
// 外部イベント（受注 / Ordering BC）
record OrderCancelled(OrderId orderId, List<OrderLineId> orderLineIds)
```

```
OrderCancelled → 引当ビューを orderLineId で引く
               → shipmentId が null の行に DeallocateStock(..., ORDER_CANCELLED)
               → shipmentId がある行には何もしない（出荷側の経路に任せる）
```

- **期限切れも同じ入口**（`EXPIRED` の判定は受注 BC 側。[H45](decisions.md#h45-期限切れを誰が判定するか)）。
  理由は外部から届いたものをそのまま `DeallocationReason` へ写像する。
- 出荷指示済みの引当は **`CancelShipment` → 引当解放（P6）** が解除する。取消の窓口は1つ（外部イベント）だが、
  **引当の状態によって行き先が分かれる**。

### なぜ引当ビューを読むのか（P2 と対称）

**外部イベントは倉庫の内部識別子を知らない。**`AllocationId` は `注文明細ID@ロケーションID` で、
どこへ引き当てたかは倉庫が決めたこと（[H24](decisions.md#h24-引当は受注の前か後か)）。だから倉庫側で引き当て直す必要がある。

| | 受けるイベント | すること | 読むビュー |
|---|---|---|---|
| **P2** 引当 | 受注が受け付けられた | 既存引当を復元し、不足を引き当てる | 引当ビュー・引当可能在庫ビュー |
| **P7** 注文取消解放 | 注文が取り消された | 出荷指示の無い引当を解除する | 引当ビュー |

**リードモデルを読むポリシーは受注 BC 由来の2本**（あとは棚卸凍結（P5）が対象在庫の列挙で読む）。

### なぜ出荷指示済みを除くのか

無条件に解除すると、**ピッキング済みの引当が消える**。その後の `StockPicked` を受けた出庫反映（P3）が
`IssueStock` を送っても、在庫集約から見れば「未知の引当ID」なので**イベントを発行せず黙って無視する**
（[H30](decisions.md#h30-ポリシーの二重発火にどう備えるか)）——**在庫が減らないまま出荷が完了する**。

`shipmentId` が null（＝まだ出荷指示が無い）なら、ピッキングは始まりようがないので安全。

### 失敗と二重発火

- **`DeallocateStock` は冪等**（H17）なので、同じ `OrderCancelled` を2回処理しても引当済は1回ぶんしか戻らない。
  **発行元が引当解放（P6）・注文取消解放（P7）の2つになっても壊れない**のは、この冪等性のおかげ。
- 凍結中でも通る（物理を動かさない。H11）ので、**注文取消解放（P7）は棚卸干渉ビューに書かない**（引当 P2・引当解放 P6 と同じ）。
- **★既知の穴**: 引当ビューの投影が遅れていると、P7 が引当を見つけられず**取りこぼす**。
  引当（P2）は在庫集約が弾いて自己修正するが（H26）、注文取消解放（P7）には弾く相手がいない——「引当が1件も無い」は
  **引き当てられなかった注文**でも起こるので、例外を投げる条件が作れない。M3+ 改修シナリオ候補。

### テスト骨子（イベント入力 → 送られるコマンド）

1. 出荷指示の無い引当があると、`DeallocateStock(ORDER_CANCELLED)` が送られる
2. **出荷指示済みの引当には何も送らない**（出荷側の経路に任せる）★
3. 明細が複数のロケーションに割れていれば、**その数だけ**送られる（1:N・状態なし）
4. 引当が1件も無ければ**何も送らない**（引き当てられなかった注文）
5. 同じイベントを2回処理しても、2回目は在庫側が冪等に無視する
...
graph LR
    ...
    R6["P7 解除対象の特定<br/>（出荷指示の無い引当）"]:::reader
    R3["StartStocktake の前段検証<br/>（重複開始を弾く）"]:::reader
    R4["人（履歴・棚卸レポート・干渉の処理）"]:::reader
    R5["人（出荷指示の明細組み立て<br/>・ピッキング担当）"]:::reader
...
- **読み手がポリシーであるビューと、人であるビューが分かれている**。引当可能在庫ビューは判断の入力なので
  **結果整合の遅れが失敗率に直結**し、在庫元帳・棚卸差異は多少遅れても困らない。
- **引当ビューだけが両方に読まれる**。引当ポリシー（P2）が再計画のために読み、**注文取消解放ポリシー（P7）**が
  解除対象を特定するために読み（[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）、人は
  **出荷指示の明細を組み立てる**とき（注文明細IDで引く）と、**ピッキング担当が何をどこから取るか**を
  見るとき（出荷IDで引く）に読む。②の**ピックリストビュー**の役割は後者を引き継いだもの
  （[H41](decisions.md#h41-リードモデルの顔ぶれを絞った基準)）。
  **出荷担当は読まない**——`ShipStock` に要る情報は出荷集約の中で確定している。
...
- **`StartStocktake` の前段検証は集約の受付ゲートではない**。棚卸集約は他の棚卸を知らないので、
...
読み手: 引当（P2）の引当先選定（H25 best-fit）／棚卸凍結（P5）の対象列挙（H27）／`StartStocktake` の前段バリデーション（H23）。
...
- **クエリは候補を取るだけ**で、選定規則（賄える棚の best-fit ／賄える棚が無ければ少ない順に使い切る ／
  非凍結だけで足りるなら棚卸中を使わない）は**ポリシーが計画として組み立てる**（[引当（P2）の手順](#手順明細1件あたり)）。
...
読み手: 出荷明細の組み立て（`orderLineId` で引く）／ピッキング担当の作業リスト（`shipmentId` で引く・[H41](decisions.md#h41-リードモデルの顔ぶれを絞った基準)）／引当（P2）の再計画（H26）／**注文取消解放（P7）の解除対象の特定**（`orderLineId` で引き `shipmentId` が null の行・[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)）。
...
| `sourceReceiptId` / `sourcePutAwayTotal` | 文字列? / 非負整数? ＋**複合一意制約** | 起点の格納を指すドメインの自然キー。**格納伝播（P1）の二重計上の検出用**（[H42](decisions.md#h42-p1-の二重計上を検出する起点識別子)） |
...
> **単発伝播の3本（P1・P3・P4）がコマンド失敗を捕まえて直接書く**。
> リードモデルというより**運用ワークリスト**。捨てても在庫は正しい（真実の源泉はイベントストアだけ）。
>
> **だから再構築の対象外にする**（[H48](decisions.md#h48-棚卸干渉ビューの更新をどのグループが担うか)）。
> `counted` の更新も含め**書き込みはすべてポリシー側の processing group** で行い、
> リードモデル再構築（truncate ＋ トークン巻き戻し）はこのテーブルに触らない。
> **再構築の影響を受けない**ので、未対処分が消える心配もない。
...
| `counted` | 真偽 | **`StockCounted` / `StocktakeClosed` 由来**（イベント）。その棚を数えたか。**更新はポリシー側のグループ**（H48） |
| `resolved` / `resolvedAt` | | **人が打つ**（非イベント）。再投入・破棄のどちらでも完了 |
| `lastEventPosition` | | `counted` 列の更新にだけ効く（イベント由来なので二重適用を弾く必要がある） |

- `counted` が要る理由: 棚卸が中断（`ABORTED`）でその棚を数えなかった場合は**必ず再投入**になる
  （勝手に治る道がない）。人の二択の判断材料（H22 の帰結）。
- **イベント由来の列と非イベント由来の列が同居する**唯一のビュー。その特殊性は
  **processing group の所属**という1点に集約してある（[H48](decisions.md#h48-棚卸干渉ビューの更新をどのグループが担うか)）——
  分ける基準は「再構築してよいか」で、`counted` は**再構築してはいけない側**に落ちる
  （行がポリシー由来なので、再構築しても `counted` を立てる相手がいない）。
...
9. 在庫元帳ビュー: **同じ格納（`入荷ID` × `格納累計`）から2本目の計上が来ると複合一意制約で弾かれる**
   （格納伝播（P1）の二重計上の検出。H30 / [H42](decisions.md#h42-p1-の二重計上を検出する起点識別子)）
...
| 成果物 | 状態 |
|---|---|
| スライス①〜⑤ | 集約4つ・ポリシー6本・サーガ1本・リードモデル5種（冒頭の表）。**M2 完了時点の記録**——M3-a 着手前の見直しで注文取消解放（P7）が加わった（[H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)） |
