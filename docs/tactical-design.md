...
- **在庫（コア）が受け側に集中している**。入口（格納伝播（P1）計上）・出口（出庫反映（P3）払出）・訂正（棚卸反映（P4）調整）・
  予約と解放（P2 / P6 / P7）がすべて在庫へ集まる形が、コアサブドメインの位置をそのまま表している。
...
- **受注 BC 由来が2本で対になっている**（P2 引当 / P7 注文取消解放）。どちらも外部イベントが倉庫の内部識別子を
  ...
  取消は**出荷指示済みなら出荷側**（`CancelShipment` → P6）、**指示前なら在庫側**（P7）へ分かれる。
...
- **明細の実在は[前段バリデーション](#前段バリデーション集約の外側)が確認する**（[H47](decisions.md#h47-前段バリデーションの枠組み)）。
  ...
  すり抜けると P3 の払出が黙って無視される（★残る穴。同節を参照）。
...
- **凍結が引当を止めない**のは、引当が物理を動かさないから（H11）。
  格納伝播・出庫反映・棚卸反映（P1・P3・P4）が干渉ビューを必要としたのは「現場は動いたのに帳簿が取り残される」ためで、P2 にはその形がない。
...
- **`DeallocateStock` は冪等**（H17）なので、同じ `OrderCancelled` を2回処理しても引当済は1回ぶんしか戻らない。
  **発行元が P6・P7 の2つになっても壊れない**のは、この冪等性のおかげ。
- 凍結中でも通る（物理を動かさない。H11）ので、**P7 は棚卸干渉ビューに書かない**（P2・P6 と同じ）。
