...
- **🟧イベントから👤アクターへ伸びる実線は「次の作業に取りかかる」の略記**。記法
  ...
  `在庫がピッキングされた→出荷担当` / `棚卸が開始された→棚卸担当`）。このうち**ピッキング担当・出荷担当が
  実際に見るのは引当ビュー**（[配線図](#配線何が書き誰が読むか)）で、格納担当・棚卸担当は同じ伝票
  （`ReceiptId` / `StocktakeId`）の中で作業が続くため集約をまたぐ一覧を要しない
...
graph LR
    ...
    R5["人（ピッキング担当・出荷担当）<br/>何をどこから出すか"]:::reader
...
- **引当ビューだけが両方に読まれる**。引当ポリシー（P2）が再計画のために読み、
  ピッキング担当・出荷担当が「何をどこから出すか」の作業リストとして読む
  （②の**ピックリストビュー**の役割を引き継いでいる。[H41](decisions.md#h41-リードモデルの顔ぶれを絞った基準)）。
