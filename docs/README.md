...
これで「4集約・7ポリシー・コアは在庫引当」という骨格が掴めます
（M1 時点で P1〜P5、M2 の出荷スライスで引当解放（P6）が → [H17](decisions.md#h17-宙に浮いた引当を誰が解放するか)、
M3-a 着手前に注文取消解放（P7）が加わった → [H46](decisions.md#h46-出荷指示前に注文が取り消されたときの引当解放)。
**顔ぶれと短縮名の正は [`ubiquitous-language.md`](ubiquitous-language.md)**）。
